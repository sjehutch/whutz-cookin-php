...
$qry = "SELECT * FROM `c_checkout`  where cook_id = '$user_id' and status = '1' ORDER BY `id` DESC";

$fire = mysql_query($qry) or die(mysql_error());
$rows = mysql_num_rows($fire);
 ?>
 <style>
   ...
   <div class="section white-text center" id="intro">
        <div class="row">
           ...
           <div class="col s12 m9  wow fadeIn content-box" style="color:#000;">
				<h4 class="text-left">My Received Order</h3>
					<hr/>
				<div class="row">
					<form action="#">
...
<?php  
if($rows > 0){
while($row = mysql_fetch_array($fire)){
?>
      <tr>
        ...
        <td><?php echo time_ago($row['time_ago']); ?></td>
		<td><?php echo $hh['address']; ?></td>
		<?php 
		$nn = $row['delivery_user'];
		$mm = mysql_fetch_array(mysql_query("SELECT * FROM `c_delivery`  where id = '$nn'"));
		?>
...
<center><img src="http://www.whutzcookin.com/web_cookin/images/noresult.png"></center>
<?php } ?>
    </tbody>
						</table>
